    ...
    return (
        <section className="w-full p-0 md:p-2">
          <article className=" flex flex-col items-center lg:items-start h-full justify-center lg:flex-row gap-8">
            <Image className="w-full lg:max-w-md" src={article?.thumbnail || ""} alt={article?.title || "Article image"} width={200} height={200}></Image>
            <div className="flex flex-col justify-between w-full max-w-2xl text-center gap-4 lg:text-left">
              ...
              <Link href={`/articles/${article?.id}`}>
                <Button className="lg:mr-auto mx-auto lg:mx-0 mt-5 max-w-[30%] lg:max-w-[20%]" variant="default" >Read More</Button>
              </Link>
            </div>
